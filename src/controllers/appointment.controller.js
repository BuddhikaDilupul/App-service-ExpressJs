...
exports.create = async (req, res, next) => {
  try {
    ...
    const responsepatient = await axios.get(
      `http://localhost:9897/auth/patient?ids=${req.body.patientId}`
    );
    await appointment.save();
    console.log(responsepatient.data[0].email);
    if (responsepatient) {
      let data={};
      data["email"] = responsepatient.data[0].email,
      data["name"] = responsepatient.data[0].firstName,
      data["date"] = req.body.date,
      data["totalCharges"] = req.body.hospitalCharges + req.body.doctorCharges
      console.log(data);

      await axios.post(
        `http://localhost:4042/email/appointment`, data
      );
    }
...
exports.attended = async (req, res, next) => {
  try {
    ...
    if (!appointment) {
      return res.status(httpStatus.NOT_FOUND).send("Appointment not found!!");
    }
    return res.status(httpStatus.OK).json({ appointment });
  } catch (error) {
    next(error);
